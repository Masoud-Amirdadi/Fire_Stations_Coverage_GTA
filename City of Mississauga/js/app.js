...
const CFG = {
    center: [43.59, -79.64],
    zoom: 11,
    polyCandidates: ['City Of Mississauag_Boundary.geojson']
};
const DATA_ROOT = './data/';

/* ========= Helpers ========= */
async function loadFirstExisting(fileNames) {
    for (const name of fileNames) {
        try {
            const r = await fetch(`${DATA_ROOT}${name}`);
...
/* ========= Base map & layers ========= */
const map = L.map('map', { preferCanvas: true }).setView(CFG.center, CFG.zoom);
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    maxZoom: 19, attribution: '&copy; OpenStreetMap contributors'
}).addTo(map);

// EBK raster tiles (visual + sampling source)
let EBK_TILE_Z = 14;
const ebk = L.tileLayer(`${DATA_ROOT}Raster_EBK/{z}/{x}/{y}.png`, {
    maxZoom: 18, opacity: 0.8, detectRetina: true
}).addTo(map);

// Boundary (outline, used for computations)
const boundary = L.geoJSON(null, {
    style: { color: '#000', weight: 2, fill: false, fillOpacity: 0 }
}).addTo(map);

// Stations (dot + label group)
...
(async () => {
    try {
        const polyData = await loadFirstExisting(CFG.polyCandidates);
        if (polyData) boundary.addData(polyData);

        const stationData = await (await fetch(`${DATA_ROOT}Fire_Stations.geojson`)).json();
        addStationsFromFC(stationData);

        try {
            const target = polyData ? L.geoJSON(polyData) : stations;
            map.fitBounds(target.getBounds(), { padding: [20, 20] });
        } catch { }
        setStatus(`Loaded boundary and ${stationCount()} station points. Ready.`);
    } catch (err) {
        console.error('GeoJSON load error:', err);
        alert('Could not load GeoJSON. Check file paths and run a local server.');
    }
})();
...
/* ========= Stations helpers ========= */
function addStationsFromFC(fc) {
    stations.clearLayers();
    if (!fc || !fc.features) return;
    fc.features.forEach(f => {
        const p = f.properties || {};
        // Use LANDMARKTY as in your Mississauga data (keep as-is)
        const id = (p.LANDMARKTY ?? p.STATION ?? p.ID ?? '').toString().trim() || 'ID';
        ...
        stations.addLayer(L.layerGroup([dot, label]));
    });
}
function stationArray() {
    const arr = [];
    stations.eachLayer(group => {
        let latlng = null, props = {};
...
function stationCount() {
    let n = 0; stations.eachLayer(() => n++); return n;
}
...
async function sampleEBKWeight(lon, lat) {
    try {
        ...
        return (0.2126 * r + 0.7152 * g + 0.0722 * b) / 255; // 0..1 luminance
    } catch { return 0; }
}
...
async function computeStationWeights(stationsArr, sampleR = 600) {
    const raw = [];
    for (const s of stationsArr) raw.push(await stationEBKMean(s.coord[0], s.coord[1], sampleR));
    const minW = Math.min(...raw), maxW = Math.max(...raw), rng = Math.max(1e-6, maxW - minW);
    return raw.map(w => (w - minW) / rng); // 0..1
}
...
// variable radii from EBK weights (high EBK => smaller radius)
...
function precomputeCoverVar(stationsArr, demands, radii_m) {
    const dm = demands.map(d => ({ ...d, m: lonLatToMeters(d.coord[0], d.coord[1]) }));
    const sm = stationsArr.map(s => ({ ...s, m: lonLatToMeters(s.coord[0], s.coord[1]) }));
    const cell = Math.max(...radii_m);
    const { index, keyFn } = buildDemandIndex(dm, cell);
...
/* ========= Color helpers for Voronoi fill ========= */
...
$run?.addEventListener('click', async () => {
    $run.disabled = true; setStatus('Building grid…');
    clearOutputs();

    const spacing = Math.max(50, parseInt($grid.value || '400', 10));
    const demands = buildDemandPoints(spacing);
    if (!demands.length) { setStatus('No boundary/demand found.'); $run.disabled = false; return; }

    const baseR = Math.max(1, parseInt($radius.value || '480', 10));
    const sArr = stationArray();
    if (!sArr.length) { setStatus('No stations found.'); $run.disabled = false; return; }

    const metric = $bufMetric.value;
    let cover, radii = sArr.map(() => baseR);

    if (metric === 'ebkdist') {
        setStatus('Computing EBK weights per station…');
        ...
        setStatus('Precomputing coverage (variable radii)…');
        cover = precomputeCoverVar(sArr, demands, radii);
    } else {
        setStatus('Precomputing coverage…');
    ...
    setStatus(`Set Cover: ${res.selected.length} station(s), ${coveredPct}% points covered in ${ms} ms.`);
    $run.disabled = false;
});

/* ========= Run: Voronoi ========= */
$runVor?.addEventListener('click', async () => {
    $runVor.disabled = true; setStatus('Computing territories…');
    ...
    if (metric === 'dist') {
        ...
        if (!cells.length) { setStatus('Could not build territories.'); $runVor.disabled = false; return; }
        if (shade) { setStatus('Shading by EBK…'); cells = await shadeVoronoiByEBK(cells, cellSize); }
        territories.addData({ type: 'FeatureCollection', features: cells });
        try { map.fitBounds(territories.getBounds(), { padding: [20, 20] }); } catch { }
        setStatus(`Built ${cells.length} territories${shade ? ' (EBK-shaded)' : ''}.`);
        $runVor.disabled = false; return;
    }

    const sArr = stationArray(); if (!sArr.length) { setStatus('No stations.'); $runVor.disabled = false; return; }
    setStatus('Computing EBK weights per station…');
    const w = await computeStationWeights(sArr, 600);
    const lambda = parseFloat($vorLambda.value || '0.6');

    const bFC = boundary.toGeoJSON(); const bUnion = unionMany(bFC); if (!bUnion) { setStatus('Bad boundary.'); $runVor.disabled = false; return; }
    const bb = turf.bbox(bUnion);
    if (cellSize > 300) cellSize = 300;
    setStatus('Generating assignment grid…');
    ...
    setStatus('Assigning cells (EBK + Distance)…');
    const colored = [];
    ...
    setStatus('Merging cells per station…');
    ...
    let features = polys;
    if (shade) { setStatus('Shading by EBK…'); features = await shadeVoronoiByEBK(polys, cellSize); }

    territories.addData({ type: 'FeatureCollection', features });
    try { map.fitBounds(territories.getBounds(), { padding: [20, 20] }); } catch { }
    setStatus(`Built ${features.length} EBK+Distance territories.`);
